...
;    cmd = execute('SSTI_2DTC_'+strtrim(i,2)+' = fltarr(nb_pts, nb_pts, tdim)')
	...
	WHILE (imax-imin+1 GT nb_pts) DO BEGIN
	  print, 't:',j,' imin/imid/imax/idim:', imin, imid, imax, imax-imin+1, ' xdim:', xdim_sstm      
	...
	WHILE (jmax-jmin+1 GT nb_pts) DO BEGIN
	  print, 't:',j,' jmin/jmid/jmax/jdim:', jmin, jmid, jmax, jmax-jmin+1, ' ydim:', ydim_sstm              
	...
	cmd = execute('SSTI_1D_'+strtrim(i,2)+'[j] = avg(SSTI_0[*,*,j], /nan)')
	cmd = execute('SSTI_1DTC_'+strtrim(i,2)+'[j] = avg(tmp[imin:imax,jmin:jmax], /nan)')
;	cmd = execute('SSTI_2DTC_'+strtrim(i,2)+'[((nb_pts-1)/2-(imid-imin)):((nb_pts-1)/2+(imax-imid)),((nb_pts-1)/2-(jmid-jmin)):((nb_pts-1)/2+(jmax-jmid)),j] = tmp[imin:imax,jmin:jmax]')
      ENDIF

    ENDFOR ; loop time
    print, 'EXTRACT SST REMSS-MWIR OK'
