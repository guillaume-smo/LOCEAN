    ...
    print, 'EXTRACT SST ALADIN-ANALYSE...'
    initncdf, file_ssta & domdef, dom_tc
    glamt = glamt[firstxt:lastxt,firstyt:lastyt]
    gphit = gphit[firstxt:lastxt,firstyt:lastyt]
    tdim  = tdim_ssta
    nb_pts = ceil(2.*radius / res_ssta) & help, nb_pts
    IF nb_pts MOD 2 EQ 0 THEN nb_pts = nb_pts-1 & help, nb_pts
    cmd = execute('SSTA_2DTC_'+strtrim(i,2)+' = fltarr(nb_pts, nb_pts, tdim)')
    cmd = execute('SSTA_1DTC_'+strtrim(i,2)+' = fltarr(tdim)')
    cmd = execute('SSTA_1D_'+strtrim(i,2)+' = fltarr(tdim)')

    FOR j = 0, tdim-1 DO BEGIN

      ; SELECTION ZONE TC
      dist_tc = reform(map_npoints(lon_mslp_0[j],lat_mslp_0[j],glamt,gphit))/1000.
      dist_tc = reform(dist_tc,xdim_ssta,ydim_ssta)
...
;      print, 'jdim:', jmax-jmin+1
      ...
      cmd = execute('SSTA_1D_'+strtrim(i,2)+'[j] = avg(SSTA_0[*,*,j], /nan)')
      cmd = execute('SSTA_1DTC_'+strtrim(i,2)+'[j] = avg(tmp[imin:imax,jmin:jmax], /nan)')
      cmd = execute('SSTA_2DTC_'+strtrim(i,2)+'[*,*,j] = tmp[imin:imax,jmin:jmax]')

    ENDFOR ; loop time
    print, 'EXTRACT SST ALADIN-ANALYSE OK'
