    ...
    print, 'EXTRACT SST REMSS-MW...'
    initncdf, file_sstm & domdef, dom_tc
    glamt = glamt[firstxt:lastxt,firstyt:lastyt]
    gphit = gphit[firstxt:lastxt,firstyt:lastyt]
    tdim  = tdim_sstm
    nb_pts = ceil(2.*radius / res_sstm) & help, nb_pts
    IF nb_pts MOD 2 EQ 0 THEN nb_pts = nb_pts-1 & help, nb_pts
;    cmd = execute('SSTM_2DTC_'+strtrim(i,2)+' = fltarr(nb_pts, nb_pts, tdim) + !VALUES.F_NAN')
    cmd = execute('SSTM_1DTC_'+strtrim(i,2)+' = fltarr(tdim) + !VALUES.F_NAN')
    cmd = execute('SSTM_1D_'+strtrim(i,2)+'   = fltarr(tdim) + !VALUES.F_NAN')

    FOR j = 0, tdim-1 DO BEGIN

      ; SELECTION ZONE TC
      dist_tc = reform(map_npoints(lon_mslp_0[j],lat_mslp_0[j],glamt,gphit))/1000.
      dist_tc = reform(dist_tc, xdim_sstm, ydim_sstm)
      ind_distmin = where(dist_tc EQ MIN(dist_tc, /NAN))
      IF n_elements(ind_distmin) GT 1 THEN ind_distmin = MAX(ind_distmin, /NAN)
      imid = (array_indices(SSTM_0[*,*,j], ind_distmin))[0]
      jmid = (array_indices(SSTM_0[*,*,j], ind_distmin))[1]
	...
	cmd = execute('SSTM_1D_'+strtrim(i,2)+'[j] = avg(SSTM_0[*,*,j], /nan)')
	cmd = execute('SSTM_1DTC_'+strtrim(i,2)+'[j] = avg(tmp[imin:imax,jmin:jmax], /nan)')
  ;	cmd = execute('SSTM_2DTC_'+strtrim(i,2)+'[((nb_pts-1)/2-(imid-imin)):((nb_pts-1)/2+(imax-imid)),((nb_pts-1)/2-(jmid-jmin)):((nb_pts-1)/2+(jmax-jmid)),j] = tmp[imin:imax,jmin:jmax]')
      ENDIF

    ENDFOR ; loop time
    print, 'EXTRACT SST REMSS-MW OK' 
