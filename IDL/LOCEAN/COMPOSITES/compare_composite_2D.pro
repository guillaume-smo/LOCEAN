...
ifin1 = where(finite(d1_pres1) EQ 1 AND finite(d1_max_wnd1) EQ 1) & help, ifin1
...
iok1 = intersect(ifin1,idom1) & help, iok1
iwind1 = where(d1_max_wnd1 GE windbin[0] AND d1_max_wnd1 LE windbin[1]) & help, iwind1
iok1 = intersect(iok1,iwind1) & help, iok1
print, correlate(d1_pres1[iok1],d1_max_wnd1[iok1])
d1_pres_smooth1 = ts_smooth(d1_pres1[iok1],5)
d1_wind_smooth1 = ts_smooth(d1_max_wnd1[iok1],5)
print, correlate(d1_pres_smooth1,d1_wind_smooth1)
dpresdt1 = (shift(d1_pres_smooth1,-2) - shift(d1_pres_smooth1,2)) / 24.
dwinddt1 = (shift(d1_wind_smooth1,-2) - shift(d1_wind_smooth1,2)) / 24.
print, correlate(dpresdt1,dwinddt1)
ipwinc1 = where(dpresdt1 LT 0. AND dwinddt1 GT 0., cptpwinc1) & help, ipwinc1
ipwdec1 = where(dpresdt1 GT 0. AND dwinddt1 LT 0., cptpwdec1) & help, ipwdec1
...
ifin2 = where(finite(d1_pres2) EQ 1 AND finite(d1_max_wnd2) EQ 1) & help, ifin2
...
iok2 = intersect(ifin2,idom2) & help, iok2
iwind2 = where(d1_max_wnd2 GE windbin[0] AND d1_max_wnd2 LE windbin[1]) & help, iwind2
iok2 =  intersect(iok2,iwind2) & help, iok2
print, correlate(d1_pres2[iok2],d1_max_wnd2[iok2])
d1_pres_smooth2 = ts_smooth(d1_pres2[iok2],5)
d1_wind_smooth2 = ts_smooth(d1_max_wnd2[iok2],5)
print, correlate(d1_pres_smooth2,d1_wind_smooth2)
dpresdt2 = (shift(d1_pres_smooth2,-2) - shift(d1_pres_smooth2,2)) / 24.
dwinddt2 = (shift(d1_wind_smooth2,-2) - shift(d1_wind_smooth2,2)) / 24.
print, correlate(dpresdt2,dwinddt2)
ipwinc2 = where(dpresdt2 LT 0. AND dwinddt2 GT 0., cptpwinc2) & help, ipwinc2
ipwdec2 = where(dpresdt2 GT 0. AND dwinddt2 LT 0., cptpwdec2) & help, ipwdec2



FOR iint = 0, n_elements(intlist)-1 DO BEGIN
int=intlist[iint]
IF int EQ 'inc' THEN iok1 = ipwinc1 
IF int EQ 'dec' THEN iok1 = ipwdec1
IF int EQ 'inc' THEN iok2 = ipwinc2 
IF int EQ 'dec' THEN iok2 = ipwdec2
...
var_name = varlist[ivar] & help, var_name
  ...
  IF var_name EQ 'SST' OR var_name EQ 'T2' THEN BEGIN & var_unit = '(degC)' & ENDIF
  ...
  saveimage, pathfig+'composite_norme_2D_'+var_name+'_'+strtrim(windbin[0],2)+'-'+strtrim(windbin[1],2)+'ms_'+int+'_'+var_typ+'_'+exp+'_'+freq+'_'+basin+'.gif', quality=100
  ...
  xyouts, 0.15, 0.22, 'SAMPLE SIZE: '+strtrim(n_elements(iok1),2), align = 0, /normal, color=0, charsize=1.5
  xyouts, 0.45, 0.22, 'SAMPLE SIZE: '+strtrim(n_elements(iok2),2), align = 0, /normal, color=0, charsize=1.5
  saveimage, pathfig+'composite_2D_'+var_name+'_'+strtrim(windbin[0],2)+'-'+strtrim(windbin[1],2)+'ms_'+int+'_'+var_typ+'_'+exp+'_'+freq+'_'+basin+'.gif', quality=100
