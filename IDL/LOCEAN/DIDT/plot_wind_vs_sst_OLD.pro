...
IF var_nam EQ 'SST' THEN d1_var = d1_var-273.15 & help, d1_var
...
IF param EQ 'COUPLED' THEN plot, bin_mid[iok],pdf_wind_var_int[iok], $
xrange=[min(bin_mid),max(bin_mid)],xtitle=bin_unit,ytitle='(degC)',yrange=[ymin,ymax],linestyle=0
IF param EQ 'FORCED' THEN plot, bin_mid[iok],pdf_wind_var_int[iok],xrange=[min(bin_mid),max(bin_mid)],/noerase,linestyle=2,yrange=[ymin,ymax]
oplot, bin_mid[iok],pdf_wind_var_intper[0,iok], linestyle=1
oplot, bin_mid[iok],pdf_wind_var_intper[1,iok], linestyle=1
IF param EQ 'COUPLED' THEN xyouts, bin_mid[iok],25.6,strtrim(nb_binint[iok],2)
IF param EQ 'FORCED' THEN  xyouts, bin_mid[iok],25.4,strtrim(nb_binint[iok],2)
IF param EQ 'FORCED' THEN  xyouts, bin_mid[iok],25.2,strtrim(ts_int[iok,1],2)
IF param EQ 'FORCED' THEN saveimage, 'FIGS_PDF/pdf_wind_bin'+strtrim(bin_size,2)+'_'+var_nam+var_typ+'_int_'+exp+'.gif'

IF param EQ 'COUPLED' THEN window,2 ELSE wset,2
iok = where(pdf_wind_var_dec NE 0.)
IF param EQ 'COUPLED' THEN plot, bin_mid[iok],pdf_wind_var_dec[iok], $
xrange=[min(bin_mid),max(bin_mid)],xtitle=bin_unit,ytitle='(degC)',yrange=[ymin,ymax]
IF param EQ 'FORCED' THEN plot, bin_mid[iok],pdf_wind_var_dec[iok],xrange=[min(bin_mid),max(bin_mid)],/noerase,linestyle=2,yrange=[ymin,ymax]
oplot, bin_mid[iok],pdf_wind_var_decper[0,iok], linestyle=1
oplot, bin_mid[iok],pdf_wind_var_decper[1,iok], linestyle=1
IF param EQ 'COUPLED' THEN xyouts, bin_mid[iok],25.6,strtrim(nb_bindec[iok],2)
IF param EQ 'FORCED' THEN xyouts, bin_mid[iok],25.4,strtrim(nb_bindec[iok],2)
IF param EQ 'FORCED' THEN xyouts, bin_mid[iok],25.2,strtrim(ts_dec[iok,1],2)
IF param EQ 'FORCED' THEN saveimage, 'FIGS_PDF/pdf_wind_bin'+strtrim(bin_size,2)+'_'+var_nam+var_typ+'_dec_'+exp+'.gif'
