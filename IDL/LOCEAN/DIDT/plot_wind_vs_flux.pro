...
; line plot par bin
ymin = 0 
IF var_nam EQ 'HFXLH' THEN ymax = 1100

IF param EQ 'COUPLED' THEN BEGIN
window,2
iok = where(pdf_wind_var_int NE 0.)
plot, bin_mid[iok],pdf_wind_var_int[iok], xstyle=1, ystyle=1, charsize=1.5, $
...
;oplot, bin_mid[iok],pdf_wind_var_dec[iok],linestyle=0,color=250,thick=2
...
plot, bin_mid[iok],pdf_wind_var_int[iok], xstyle=1, ystyle=1, charsize=1.5, /noerase, $
xrange=[bin[min(iok)],bin[max(iok)+1]],yrange=[ymin,ymax]
oplot, bin_mid[iok],pdf_wind_var_int[iok],linestyle=0,color=250, thick=2
;oplot, bin_mid[iok],pdf_wind_var_dec[iok],linestyle=0,color=200,thick=2
;xyouts, 0.15, 0.85, 'FORCED-INTEN', /normal, color=250, charsize=1.5
;xyouts, 0.15, 0.75, 'FORCED-DECAY', /normal, color=200, charsize=1.5
oplot, bin_mid[iok],pdf_wind_var_intper[0,iok], linestyle=2, color=250, thick=1
oplot, bin_mid[iok],pdf_wind_var_intper[1,iok], linestyle=2, color=250, thick=1
