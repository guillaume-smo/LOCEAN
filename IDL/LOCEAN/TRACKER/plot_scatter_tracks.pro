...
restore, '/net/adonis/usr/adonis/varclim/gslod/IDL/TRACKER/EXP_'+expname+'/DATA/tracker_light_'+temptype+'_RVM_NICO_MASK_u'+ strtrim(long(ucrit),2) +'_v'+ strtrim(long(vorcrit*100000),2) +'_t'+ strtrim(long(tempcrit*10),2) +'_r'+ strtrim(long(vorelax*100000),2) +'_'+ period +'.idl', /VERBOSE
pathfig = 'FIGS_SCATTER_'+temptype+'/' & spawn, 'echo "creation du repertoire '+ pathfig +'"' & spawn, 'mkdir -p '+ pathfig
...
ave_binrvm  = fltarr(n_elements(centerbin))+!values.f_nan

FOR i = 0, n_elements(centerbin)-1 DO BEGIN
  indok = where(uv10cycn GE borderbin[i] AND uv10cycn LE borderbin[i+1], cntok)
  IF cntok GT 0 THEN BEGIN
    ave_binuv10[i] = mean(uv10cycn[indok], /NAN)
    ave_binmslp[i] = mean(mslpcycn[indok], /NAN)
    ave_binvort[i] = mean(vorcycn[indok], /NAN)
    ave_binanot[i] = mean(anotcycn[indok], /NAN)
    ave_binrvm[i]  = mean(rvmcycn[indok], /NAN)    
  ENDIF
  nb_binuv10[i] = cntok
ENDFOR
...
coef_uvrvm  = regress(ave_binuv10[indok], ave_binrvm[indok],  const = cst_uvrvm)


;x = [10,60] & y = x * coef_uvrvm[0] + cst_uvrvm
;plot, uv10cycnn, rvmCYCn, PSYM=3, xrange=[10,60], yrange=[0,400], xtitle='10m wind (m/s)', ytitle='RMW (km)', title='nb points: '+strtrim(n_elements(where(finite(uv10cycnn) EQ 1)),2)
;oplot, ave_binuv10[where(ave_binuv10 NE 0.)], ave_binrvm[where(ave_binuv10 NE 0.)], color=100, thick=2
;oplot, x,y, color=200, thick=2
;xyouts, 0.8, 0.8, 'COEF: '+strtrim(coef_uvrvm[0],2), charsize = 1.5, /NORMAL
;xyouts, 0.8, 0.7, 'CST : '+strtrim(cst_uvrvm,2), charsize = 1.5, /NORMAL
;saveimage, pathfig+'SCATTER_TRACKS_UV10M_RVM_'+expname+'_u'+ strtrim(long(ucrit),2) +'_v'+ strtrim(long(vorcrit*100000),2) +'_t'+ strtrim(long(tempcrit*10),2) +'_r'+ strtrim(long(vorelax*100000),2) +'_'+ period +'.gif'


x = [10,60] & y = x * coef_uvmslp[0] + cst_uvmslp
plot, uv10cycn, mslpcycn, PSYM=3, xrange=[10,60], yrange=[930,1020], xtitle='10m wind (m/s)', ytitle='sea level pressure (hPa)', title='nb points: '+strtrim(n_elements(where(finite(uv10cycn) EQ 1)),2)
...
saveimage, pathfig+'SCATTER_TRACK_UV10M_MSLP_'+expname+'_u'+ strtrim(long(ucrit),2) +'_v'+ strtrim(long(vorcrit*100000),2) +'_t'+ strtrim(long(tempcrit*10),2) +'_'+ period +'.gif'
...
plot, uv10cycn, anotcycn, PSYM=3, xrange=[10,60], yrange=[0,6], xtitle='10m wind (m/s)', ytitle='temperature anomaly (degC)', title='nb points: '+strtrim(n_elements(where(finite(uv10cycn) EQ 1)),2)
...
saveimage, pathfig+'SCATTER_TRACK_UV10M_ANOMT_'+expname+'_u'+ strtrim(long(ucrit),2) +'_v'+ strtrim(long(vorcrit*100000),2) +'_t'+ strtrim(long(tempcrit*10),2) +'_'+ period +'.gif'
...
plot, uv10cycn, vorcycn, PSYM=3, xrange=[10,60], yrange=[0,0.004], xtitle='10m wind (m/s)', ytitle='relative vorticity (s-1)', title='nb points: '+strtrim(n_elements(where(finite(uv10cycn) EQ 1)),2)
...
saveimage, pathfig+'SCATTER_TRACK_UV10M_VOR800_'+expname+'_u'+ strtrim(long(ucrit),2) +'_v'+ strtrim(long(vorcrit*100000),2) +'_t'+ strtrim(long(tempcrit*10),2) +'_'+ period +'.gif'
