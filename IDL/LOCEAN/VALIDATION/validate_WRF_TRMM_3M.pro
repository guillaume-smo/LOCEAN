PRO validate_WRF_TRMM_3M
  ...
  FOR m = 0, n_elements(month_list)-1 DO BEGIN
    ...
    IF jRAIN THEN BEGIN
      ;;;;;;;;;;;;;;;;;;;;;;  OBS ;;;;;;;;;;;;;;;;;;;;;;; 
      ;-------------------------------------------------;
      pathout  = '/Volumes/TIME_MACHINE/DATA/PRECIP-TRMM_3B43/'
      pathfile = 'precip_trmm_clim_monthly_1998-2010.nc'
      initncdf, pathout+pathfile, /fullcgrid
      domdef, box

      vari=read_ncdf('pcp',0,11,/timestep, filename=pathout+pathfile, /nostruct) & help, vari
      ...
      rain_o = vari[*,*]*24. & help, rain_o & vari = 0
      ...
      IF month EQ 'JJA' THEN vari = total(vari[*,*,5:7],3) / 3.
      IF month EQ 'SON' THEN vari = total(vari[*,*,8:10],3) / 3.
      rain_w = vari[*,*] & help, rain_w & vari = 0
