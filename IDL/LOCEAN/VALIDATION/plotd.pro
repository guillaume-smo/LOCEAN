...
plot_typ = '2D'
var_list = ['SST']
;exp_list = ['COUPLED_SW2_KF','COUPLED_SW2_BMJ','CPL_KFOLD_V33','CPL_BMJ_V33','CPL_BMJCAM_V33']
;exp_list = ['COUPLED_SW2_KF','COUPLED_SW2_BMJ','CPL_KFOLD_V33','CPL_KFCAM_V33','CPL_BMJ_V33','CPL_BMJCAM_V33']
exp_list = ['ERA-I','COUPLED_SW2_KF','COUPLED_SW2_BMJ','CPL_BMJ_W33N32','CPL_NSAS_V33', 'CPL_TDK_V33']
IF plot_typ EQ '1D' THEN bas_list = ['SBND','EBND','BOB','ARS','SWI','SEI','SUM'] ELSE bas_list = ['IO']
IF plot_typ EQ '1D' THEN period = [19900101d,19921231d] ELSE period = [19920101d,19920331d]
...
file = varn+'_1990-1992.nc'
IF expn EQ 'CPL_TDK_V33' THEN file = varn+'_1990-1992.nc'
IF expn EQ 'CPL_NSAS_V33' THEN file = varn+'_1990-1992.nc'
...
IF basn EQ 'SBND' THEN box = [30,130,-35,-34]
IF basn EQ 'EBND' THEN box = [142,143,-30,25]
...
IF plot_typ EQ '1D' AND expn NE 'TROPFLUX' AND expn NE 'NOAA' THEN BEGIN
  mask = reform(reform(mask, nxt*nyt)#replicate(1,cntok), nxt, nyt, cntok)
  var  = var * mask
ENDIF
IF plot_typ EQ '1D' THEN var = m_mean(m_mean(var,dim=2,/nan),dim=1,/nan) & help, var
...
IF plot_typ EQ '1D' THEN BEGIN
  IF expn EQ 'TROPFLUX' OR expn EQ 'NOAA' THEN $
  varsmooth = ts_smooth(var,11) ELSE varsmooth = ts_smooth(var,41) & help, varsmooth
  var = varsmooth
ENDIF
IF plot_typ EQ '2D' THEN var  = m_mean(var[*,*,iok],dim=3,/nan) & help, var
IF plot_typ EQ '2D' AND expn NE 'TROPFLUX' AND expn NE 'NOAA' THEN var = var * mask

IF plot_typ EQ '1D' THEN BEGIN
  IF expn EQ 'TROPFLUX' OR expn EQ 'NOAA' THEN days = findgen(cntok) ELSE days = findgen(cntok)/4
  IF expn EQ 'TROPFLUX' OR expn EQ 'NOAA' THEN month = findgen(cntok)/30.5+1 ELSE month = findgen(cntok)/(4*30.5)+1
...
IF plot_typ EQ '2D' THEN BEGIN
  IF varn EQ 'OLR' THEN BEGIN & ymin = 190 & ymax = 290 & ENDIF
  IF varn EQ 'SST' THEN BEGIN & ymin = 20 & ymax = 32 & ENDIF
  plt, var, ymin, ymax, /nocontour, /realcont, lct=34, $
  title='EXP: '+expn+' PERIOD: '+strtrim(long(period[0]),2)+'-'+strtrim(long(period[1]),2),$
  subtitle=varn, xtitle=' ', ytitle=' ', charsize=1.2, cb_charsize=1.2, format='(F5.1)'
...
;IF plot_typ EQ '1D' THEN saveimage, varn+'_1D_1990-1991_'+basn+'.gif'
stop
ENDFOR
ENDFOR
