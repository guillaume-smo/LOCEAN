...
bas_list = ['TMPBMJ']
freq     = '6H' ; frequence des fichiers input "modele" (6H/DAILY/MONTHLY)

; KF
;date_ok = 20050122.25d

; BMJ
;date_ok = 19910304.75d
date_ok = 20090106.50d
...
IF basn EQ 'TMPKF' THEN box= [114,123,-10.5,-19.5]

;IF basn EQ 'TMPBMJ' THEN box= [30.,45.,-30.,-20.]
IF basn EQ 'TMPBMJ' THEN box= [69.5,78.5,-18,-9]
...
IF expn EQ 'COUPLED_SW2_BMJ' OR expn EQ 'COUPLED_SW2_KF' OR expn EQ 'FORCED_SW2_KF' OR expn EQ 'CPL_TDK_V33' OR expn EQ 'CPL_NSAS_V33' THEN BEGIN
  IF gridobs THEN BEGIN & initncdf, path+file & domdef, box & ENDIF ELSE BEGIN
    ...
    IF where(landmask EQ 1) NE -1 THEN mask[where(landmask EQ 1)] = !VALUES.F_NAN & help, mask 
  ENDELSE
...
IF varn EQ 'UV10' THEN BEGIN
  tmp = execute('U10_'+expn+' = var1 & help, U10_'+expn)
  tmp = execute('V10_'+expn+' = var2 & help, V10_'+expn)
ENDIF
tmp = execute(strlowcase(varn)+'_'+expn+' = var & help, '+strlowcase(varn)+'_'+expn)
tmp = execute('mask_'+strlowcase(varn)+'_'+expn+' = mask & help, mask_'+strlowcase(varn)+'_'+expn)
...
; ecriture netcdf fichier clim monthly grid obs
...
plt, RAIN_COUPLED_SW2_KF*MASK_RAIN_COUPLED_SW2_KF, /nocont, format='(F5.0)', min=0, max=1800
ajoutvect, {u:{a:U10_COUPLED_SW2_KF, g:'T'},v:{a:V10_COUPLED_SW2_KF, g:'T'}}, normeref=40, cmref=1, vectthick=1, unvectsur=[2,2]
ENDIF
...
STOP
t=2
plt, RAIN_COUPLED_SW2_BMJ[*,*,2]*MASK_RAIN_COUPLED_SW2_BMJ, /nocont, format='(F5.0)', min=0, max=1800
ajoutvect, {u:{a:U10_COUPLED_SW2_BMJ[*,*,1], g:'T'},v:{a:V10_COUPLED_SW2_BMJ[*,*,1], g:'T'}}, normeref=40, cmref=1, vectthick=1, unvectsur=[2,2]
ENDIF
