...
;-------------------------------------------------------------------------------------------------
  ...
  IF zone EQ 'ALL' OR zone EQ 'TROP' THEN BEGIN

    var_zonmean_0 = MEAN(var_mean_0, DIMENSION=1, /NAN)
    FOR e = 1, n_elements(exp_list) -1 DO cmd = execute( 'var_zonmean_'+strtrim(e,2)+' = MEAN(var_mean_'+strtrim(e,2)+'_gridobs, DIMENSION=1, /NAN)' )

    var_plot = 'var_zonmean'
    @def_plot_minmax
    ...
    FOR e = 1, n_elements(exp_list) -1 DO cmd = execute( 'err_zonmean_'+strtrim(e,2)+' = var_zonmean_'+strtrim(e,2)+' - var_zonmean_0' )

    var_plot = 'err_zonmean'
    @def_plot_minmax
