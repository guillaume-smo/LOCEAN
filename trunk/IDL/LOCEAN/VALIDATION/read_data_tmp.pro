...
iok = where(date EQ date_ok) & print, iok
...
IF varn EQ 'UV10' THEN BEGIN
  IF expn EQ 'COUPLED_SW2_BMJ' OR expn EQ 'COUPLED_SW2_KF' OR expn EQ 'FORCED_SW2_KF' OR expn EQ 'CPL_TDK_V33' OR expn EQ 'CPL_NSAS_V33' THEN BEGIN
    var1 = read_ncdf('U10', iok-1, iok+1, /timestep, filename=path+file, /nostruct)
    var2 = read_ncdf('V10', iok-1, iok+1, /timestep, filename=path+file, /nostruct)
...
IF varn EQ 'RAIN' THEN BEGIN
  IF freq EQ '6H' AND expn EQ 'COUPLED_SW2_KF' THEN BEGIN
    var1 = read_ncdf('RAINC', iok, iok, /timestep, filename=path+'RAINC_6H_1990-2009.nc', /nostruct) * 4.
    var2 = read_ncdf('RAINNC', iok, iok, /timestep, filename=path+'RAINNC_6H_1990-2009.nc', /nostruct) * 4. ; mm/6H -> mm/d
;    var2 = 0.
    var  = var1 + var2 >0
;    var = read_ncdf('RAIN', iok, iok, /timestep, filename=path+file, /nostruct)
  ENDIF
  IF freq EQ '6H' AND expn EQ 'COUPLED_SW2_BMJ' THEN BEGIN
    var1 = read_ncdf('RAINC', iok-1, iok+1, /timestep, filename=path+'RAINC_6H_1990-2009.nc', /nostruct) * 4.
    var2 = read_ncdf('RAINNC', iok-1, iok+1, /timestep, filename=path+'RAINNC_6H_1990-2009.nc', /nostruct) * 4. ; mm/6H -> mm/d
;    var2 = 0.
    var  = var1 + var2 >0
  ENDIF
