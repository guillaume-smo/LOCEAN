...
ENDIF ELSE BEGIN
  ...
  file_in   = 'tracker_light_'+temptype+'_new_cst_radius_u'+ strtrim(long(ucrit),2) +'_v'+ strtrim(long(vorcrit*100000),2) +'_t'+ strtrim(long(tempcrit*10),2) +'_r'+ strtrim(long(vorelax*100000),2) +'_'+ period +'.idl' & help, file_in
...
isio = where(loncycn[*,0] GE 30 AND loncycn[*,0] LE 130 AND latcycn[*,0] GE -30 AND latcycn[*,0] LE 0, cntsio)
...
plt, glamt, /nodata, /realcont, min=min_wind, max=max_wind, title=expname+' light '+temptype+' - number of TCs in '+strtrim(y,2)+': '+strtrim(cntok,2)+' ('+strtrim(cntsio,2)+'SI+'+strtrim(cntnio,2)+'NI)', subtitle='', xtitle='', ytitle=''
