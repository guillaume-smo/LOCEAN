...
IF var_typ EQ 'DUR' THEN var_xy = dxy_var_wsc_rot & help, var_xy
...
;    IF iok[0] NE -1 THEN ts_int[i,*] = RS_TEST(reform(dpresdtint_cpl[i,where(dpresdtint_cpl[i,*] NE 0)]), $
;                                               reform(dpresdtint_frc[i,where(dpresdtint_frc[i,*] NE 0)]))
;    iok = where(dpresdtdec_cpl[i,*] NE 0 AND dpresdtdec_frc[i,*] NE 0, cntok)
;    IF iok[0] NE -1 THEN ts_dec[i,*] = RS_TEST(reform(dpresdtdec_cpl[i,where(dpresdtdec_cpl[i,*] NE 0)]), $
;                                               reform(dpresdtdec_frc[i,where(dpresdtdec_frc[i,*] NE 0)]))
    ts_dpresdtall[i,*] = RS_TEST([reform(dpresdtint_cpl[i,where(dpresdtint_cpl[i,*] NE 0)]),  $
                                  reform(dpresdtdec_cpl[i,where(dpresdtdec_cpl[i,*] NE 0)])], $
                                 [reform(dpresdtint_frc[i,where(dpresdtint_frc[i,*] NE 0)]),  $
                                  reform(dpresdtdec_frc[i,where(dpresdtdec_frc[i,*] NE 0)])])
    ts_dwinddtall[i,*] = RS_TEST([reform(dwinddtint_cpl[i,where(dwinddtint_cpl[i,*] NE 0)]),  $
                                  reform(dwinddtdec_cpl[i,where(dwinddtdec_cpl[i,*] NE 0)])], $
                                 [reform(dwinddtint_frc[i,where(dwinddtint_frc[i,*] NE 0)]),  $
                                 reform(dwinddtdec_frc[i,where(dwinddtdec_frc[i,*] NE 0)])])
  ENDFOR
ENDIF
STOP
...
;saveimage, 'FIGS_PDF/pdf_'+bin_name+'-'+var_typ+'_bin'+strtrim(bin_size,2)+'_dUVdt_all_'+exp+'.gif'
ENDIF
...
ymin = -2.5 & ymax = 2.5
iok = where(dwinddtall_bin NE 0.)
plot, bin_mid[iok], dwinddtint_bin[iok], xstyle=1, ystyle=1, $
xrange=[bin[min(iok)],bin[max(iok)+1]],xtitle=bin_name+' '+bin_unit,ytitle=var_nam+' '+var_uni,yrange=[ymin,ymax]
oplot, bin_mid[iok], dwinddtint_bin[iok], thick=2, color=50
oplot, bin_mid[iok], dwinddtdec_bin[iok], thick=2, color=250
xyouts, 0.15, 0.90, 'COUPLED-INTEN', /normal, color=50, charsize=1.5
xyouts, 0.15, 0.85, 'COUPLED-DECAY', /normal, color=250, charsize=1.5
oplot, [bin_deb,bin_fin],[0,0]
tmpall = dwinddtall_bin
ENDIF

IF param EQ 'FORCED' THEN BEGIN
wset,3
iok = where(tmpall NE 0. )
plot, bin_mid[iok], dwinddtint_bin[iok], xstyle=1, ystyle=1, /noerase, $
xrange=[bin[min(iok)],bin[max(iok)+1]],yrange=[ymin,ymax]
oplot, bin_mid[iok], dwinddtint_bin[iok], thick=2, color=100
oplot, bin_mid[iok], dwinddtdec_bin[iok], thick=2, color=200
xyouts, 0.15, 0.80, 'FORCED-INTEN', /normal, color=100, charsize=1.5
xyouts, 0.15, 0.75, 'FORCED-DECAY', /normal, color=200, charsize=1.5
;saveimage, 'FIGS_PDF/pdf_'+bin_name+'-'+var_typ+'_bin'+strtrim(bin_size,2)+'_dUVdt_int+dec_'+exp+'.gif'
ENDIF
