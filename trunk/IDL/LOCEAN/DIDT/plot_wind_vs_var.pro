...
xrange=[min(bin_mid[iok]),max(bin_mid[iok])],xtitle=bin_name+' '+bin_unit,ytitle=var_nam+' '+var_uni,yrange=[ymin,ymax]
...
xyouts, bin_mid[iok],ymax-50,strtrim(nb_binint[iok],2)
xyouts, 0.1, 0.5, 'CPL', color=50 , /normal
xyouts, 0.1, 0.4, 'FRC', color=150, /normal
tmpint = pdf_wind_var_int
ENDIF

IF param EQ 'FORCED' THEN BEGIN
wset,2
iok = where(tmpint NE 0. AND pdf_wind_var_int NE 0.)
plot, bin_mid[iok],pdf_wind_var_int[iok], /noerase, $
xrange=[min(bin_mid[iok]),max(bin_mid[iok])],yrange=[ymin,ymax]
oplot, bin_mid[iok],pdf_wind_var_int[iok],linestyle=0,color=150, thick=2
...
saveimage, 'FIGS_PDF/pdf_wind_bin'+strtrim(bin_size,2)+'_'+var_nam+'_int_'+exp+'.gif'
ENDIF
