...
restore, pathtc + 'd1_IBTRACSv03r03_IO_'+period+'.dat', /VERBOSE
...
; IF param EQ 'COUPLED' THEN BEGIN
  ...
  pdf_wind_var_int_obs = pdf_wind_var_int
  pdf_wind_var_intper_obs = pdf_wind_var_intper
  ...
  oplot, bin_mid[iok],pdf_wind_var_dec[iok],linestyle=0, thick=2, color=250
  ...
  oplot, bin_mid[iok],pdf_wind_var_decper[0,iok], linestyle=2, thick=1, color=250
  oplot, bin_mid[iok],pdf_wind_var_decper[1,iok], linestyle=2, thick=1, color=250
...
;  window,1
;  ymin = -1 & ymax = 0.7
;  iok = where(pdf_wind_var_int_obs NE 0.)
;  plot, bin_mid[iok],pdf_wind_var_int_obs[iok], xstyle=1, ystyle=1, $
;  xrange=[bin[min(iok)],bin[max(iok)+1]],xtitle='10M WIND '+bin_unit,ytitle='COOLING (degC)',yrange=[ymin,ymax],linestyle=0, thick=2, color=0
;  oplot, bin_mid[iok],pdf_wind_var_int_obs[iok],linestyle=0,thick=2, color=50
;  oplot, bin_mid[iok],pdf_wind_var_intper_obs[0,iok], linestyle=2, thick=1, color=50
;  oplot, bin_mid[iok],pdf_wind_var_intper_obs[1,iok], linestyle=2, thick=1, color=50
...
;  saveimage, 'FIGS_PDF/pdf_wind_bin'+strtrim(bin_size,2)+'_COOLING_'+rayon+'km_int_obs+FRC_'+exp+'.gif'
