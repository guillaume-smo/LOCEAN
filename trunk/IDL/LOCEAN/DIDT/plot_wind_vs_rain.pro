...
vartmp = fltarr(n_elements(dxy_var_wsc_rot[*,*,0,0]),n_elements(dxy_var_wsc_rot[0,0,*,0]),n_elements(dxy_var_wsc_rot[0,0,0,*]))
...
ibad = where(d1_var[iokint] GE 1000.)
IF ibad[0] NE -1 THEN d1_var[iokint[ibad]] = !values.f_nan
ibad = where(d1_var[iokdec] GE 1000.)
IF ibad[0] NE -1 THEN d1_var[iokdec[ibad]] = !values.f_nan
...
; line plot par bin
IF var_nam EQ 'RAINC' THEN BEGIN & ymin = 0 & ymax = 30 & ENDIF
IF var_nam EQ 'RAINNC' THEN BEGIN & ymin = 0 & ymax = 60 & ENDIF
...
xyouts, 0.15, 0.90, var_nam+'_ICR - INT', /normal, color=50, charsize=1.5
xyouts, 0.15, 0.85, var_nam+'_ICR - DEC', /normal, color=200, charsize=1.5
