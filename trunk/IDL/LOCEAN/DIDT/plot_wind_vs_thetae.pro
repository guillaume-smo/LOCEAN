...
; lecture best-track
;restore, 'dist_tc_'+rayon+'km.idl', /verbose
...
d1_var = m_mean(m_mean(temporary(dxy_var_wsc_rot),dim=4,/nan),dim=3,/nan)
;idist = where(dist_tc GT 100.)
;d1_var = m_mean(temporary(dxy_var_wsc_rot[*,*,idist]),dim=3,/nan)
IF var_nam EQ 'Q2' OR var_nam EQ 'QS0' OR var_nam EQ 'Q2-QS0' THEN d1_var = d1_var * 1000.
IF var_nam EQ 'T2' THEN d1_var = d1_var - 273.15
...
ymin = min(pdf_wind_var_dec[iok]) & ymax = max(pdf_wind_var_int[iok])
IF var_nam EQ 'T2' THEN BEGIN & ymin = 25 & ymax = 30 & ENDIF
IF var_nam EQ 'Q2' THEN BEGIN & ymin = 19 & ymax = 23 & ENDIF
IF var_nam EQ 'QS0' THEN BEGIN & ymin = 20 & ymax = 30 & ENDIF
IF var_nam EQ 'Q2-QS0' THEN BEGIN & ymin = -6 & ymax = -1 & ENDIF
IF var_nam EQ 'T2-SST' THEN BEGIN & ymin = -1.2 & ymax = -0 & ENDIF
IF var_nam EQ 'UV10' THEN BEGIN ymin = 10 & ymax = 40 & ENDIF
IF var_nam EQ 'THETAE' THEN BEGIN ymin = 355 & ymax = 370 & ENDIF
...
xyouts, 0.15, 0.90, var_nam+'_ICR - INT', /normal, charsize=1.5, color=50
xyouts, 0.15, 0.85, var_nam+'_ICR - DEC', /normal, charsize=1.5, color=200
...
xrange=[bin[min(iok)],bin[max(iok)+1]],yrange=[ymin,ymax],linestyle=2, thick=2
