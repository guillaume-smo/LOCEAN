...
IF basin EQ 'SIO'  THEN idom1 = where(d1_lon[*,0] GT 30. AND d1_lon[*,0] LT 130. AND d1_lat[*,0] LT 0.)
help, idom1
iok1 = idom1
...
FOR ivar = 0, n_elements(varlist)-1 DO BEGIN
  ...
  var_env  = m_mean(varmoy_1[where(axe_x GE rayon_ano),*], dim=1, /nan) & help, var_env
  presenv  = m_mean(presmoy[where(axe_x GE rayon_ano),*], dim=1, /nan) & help, presenv
  var_ano  = varmoy_1
  presano  = varmoy_1
  ...
  saveimage, pathfig+'composite_RP-CG_'+var_name+'_moy_'+expname1+'_'+freq+'_'+basin+'_'+period+'.gif'

  lct,42
  pltzz, presmoy, var_ano,  /rempli, box=[0,1005],zoom=1005, min=vmin, max=vmax, $
  cell_fill=2, xtickformat='', xtitle='radius (km)', ytitle='pressure (hPa)', subtitle='', title=''
  saveimage, pathfig+'composite_RP-CG_'+var_name+'_ano'+strtrim(rayon_ano,2)+'_'+expname1+'_'+freq+'_'+basin+'_'+period+'.gif'
