...
print, '' & print, 'plot_1D_x: ', var_plot
...
;-------------------------------------------------------------------------------------------------
  ...
  var_ymean_0 = MEAN(var_mean_0, DIMENSION=2, /NAN)
  FOR e = 1, n_elements(exp_list) -1 DO cmd = execute( 'var_ymean_'+strtrim(e,2)+' = MEAN(var_mean_'+strtrim(e,2)+'_gridobs, DIMENSION=2, /NAN)' )
  ...
  splot, axe_x, var_y, xtitle='LONGITUDE', title=mask_title+' '+var_name+' MERIDIONAL AVERAGE - '+zone+' - '+period, charsize=1.5, ystyle=1, yrange=[minvar,maxvar], ytitle=var_name, lct=39, charthick=1.5, XTicklen=1, YTicklen=1, XGridStyle=1, YGridStyle=1, /nodata
...
;-------------------------------------------------------------------------------------------------
; PLOT MERIDIONAL ERROR
;-------------------------------------------------------------------------------------------------

IF n_elements(exp_list) GT 1 THEN BEGIN

  var_plot = 'err_ymean'
  print, '' & print, 'plot_1D_yz: ', var_plot

  FOR e = 1, n_elements(exp_list) -1 DO cmd = execute( 'err_ymean_'+strtrim(e,2)+' = var_ymean_'+strtrim(e,2)+' - var_ymean_0' )
  ...
  axe_x = lon_0 & help, axe_x
  cmd = execute( 'var_y = '+var_plot+'_1' ) & help, var_y
  splot, axe_x, var_y, xtitle='LONGITUDE', title=mask_title+' '+var_name+' MERIDIONAL AVERAGE ERROR - '+zone+' - '+period, charsize=1.5, ystyle=1, yrange=[minvar,maxvar], ytitle=var_name, lct=39, charthick=1.5, XTicklen=1, YTicklen=1, XGridStyle=1, YGridStyle=1, /nodata
