...
;      print, 'jdim:', jmax-jmin+1
      ...
      cmd = execute('SSTG_1D_'+strtrim(i,2)+'[j] = avg(SSTG_0[*,*,j], /nan)')
      cmd = execute('SSTG_1DTC_'+strtrim(i,2)+'[j] = avg(tmp[imin:imax,jmin:jmax], /nan)')
      cmd = execute('SSTG_2DTC_'+strtrim(i,2)+'[*,*,j] = tmp[imin:imax,jmin:jmax]')

    ENDFOR ; loop time
    print, 'EXTRACT SST GLORYS OK'
